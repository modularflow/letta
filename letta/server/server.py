...
class SyncServer(Server):
    ...
    def _add_agent(self, user_id: str, agent_id: str,
    ...
    def _load_agent(self,
        ...
        try:
            ...
            for name in agent_state.tools:
                ...
                try:
                    tool_obj = self.tool_manager.get_tool_by_name(
            ...
            self._add_agent(user_id=user_id,
                            agent_id=agent_id,
    ...
    def _command(self, user_id: str, agent_id: str,
        ...
        if command.lower() == "exit":
            ...
            command = command.strip().split()
            amount = int(
                command[1]) if len(command) > 1 and command[1].isdigit() else 0
            if amount == 0:
                letta_agent.interface.print_messages(letta_agent.messages,
                                                     dump=True)
            else:
                letta_agent.interface.print_messages(
                    letta_agent.
                    messages[-min(amount, len(letta_agent.messages)):],
                    dump=True)
            ...
            command = command.strip().split()
            pop_amount = int(
                command[1]) if len(command) > 1 and command[1].isdigit() else 3
        ...
        elif command.lower() == "rethink" or command.lower().startswith(
                "rethink "):
            # TODO this needs to also modify the persistence manager
        ...
        # No skip options
            ...
            usage = self._step(user_id=user_id,
                               agent_id=agent_id,
    ...
    def system_message(
        ...
        # Run the agent state forward
        return self._step(user_id=user_id,
                          agent_id=agent_id,
                          input_messages=message)
    ...
    def run_command(self, user_id: str, agent_id: str,
        ...
        return self._command(user_id=user_id,
                             agent_id=agent_id,
                             command=command)
    ...
    def update_agent(
        ...
        if request.memory:
            ...
            _ = self.update_agent_core_memory(
                user_id=actor.id,
                agent_id=request.id,
        ...
        if request.tools:
            ...
            tools_to_add = [
                self.tool_manager.get_tool_by_name(tool_name=tool, actor=actor)
            ...
            for tool in tools_to_remove:
                self.remove_tool_from_agent(agent_id=request.id,
                                            tool_id=tool.id,
                                            user_id=actor.id)
            for tool in tools_to_add:
                self.add_tool_to_agent(agent_id=request.id,
                                       tool_id=tool.id,
                                       user_id=actor.id)
        ...
        if request.tags is not None:
            current_tags = set(
                self.agents_tags_manager.get_tags_for_agent(
            ...
            for tag in tags_to_add:
                self.agents_tags_manager.add_tag_to_agent(
                    agent_id=letta_agent.agent_state.id, tag=tag, actor=actor)
            for tag in tags_to_remove:
                self.agents_tags_manager.delete_tag_from_agent(
                    agent_id=letta_agent.agent_state.id, tag=tag, actor=actor)
    ...
    def list_agents(self,
                    user_id: str,
                    tags: Optional[List[str]] = None) -> List[AgentState]:
        """List all available agents to a user"""
        user = self.user_manager.get_user_by_id(user_id=user_id)
        ...
        if tags is None:
            ...
            agent_ids = []
            for tag in tags:
                agent_ids += self.agents_tags_manager.get_agents_by_tag(
                    tag=tag, actor=user)

            return [
                self.get_agent_state(user_id=user.id, agent_id=agent_id)
    ...
    def get_blocks(
        ...
        return self.ms.get_blocks(user_id=user_id,
                                  label=label,
                                  template=template,
                                  template_name=name,
                                  id=id)
        ...
        if existing_blocks is not None and request.template:
            ...
            if update:
                return self.update_block(
                    UpdateBlock(id=existing_block.id, **vars(request)))
            else:
                raise ValueError(
                    f"Block with name {request.template_name} already exists")
    ...
    def get_source(self, source_id: str, user_id: str) -> Source:
        existing_source = self.ms.get_source(source_id=source_id,
                                             user_id=user_id)
    ...
    def get_source_id(self, source_name: str, user_id: str) -> str:
        existing_source = self.ms.get_source(source_name=source_name,
                                             user_id=user_id)
        if not existing_source:
            raise ValueError("Source does not exist")
    ...
    def get_archival_memory_summary(self,
    ...
    def get_agent_messages(
        ...
        if start + count < len(
                letta_agent._messages
        ):  # messages can be returned from whats in memory
            ...
            db_iterator = letta_agent.persistence_manager.recall_memory.storage.get_all_paginated(
                page_size=count, offset=start)
    ...
    def get_agent_archival(self, user_id: str, agent_id: str, start: int,
        ...
        db_iterator = letta_agent.persistence_manager.archival_memory.storage.get_all_paginated(
            page_size=count, offset=start)
    ...
    def insert_archival_memory(self, user_id: str, agent_id: str,
        ...
        return [
            letta_agent.persistence_manager.archival_memory.storage.get(
                id=passage_id) for passage_id in passage_ids
        ]

    def delete_archival_memory(self, user_id: str, agent_id: str,
                               memory_id: str):
        if self.user_manager.get_user_by_id(user_id=user_id) is None:
            raise ValueError(f"User user_id={user_id} does not exist")
        if self.ms.get_agent(agent_id=agent_id, user_id=user_id) is None:
        ...
        letta_agent.persistence_manager.archival_memory.storage.delete(
            {"id": memory_id})
    ...
    def get_agent_recall_cursor(
        ...
        assistant_message_function_kwarg: str = constants.
        ...
        if not return_message_object:
            ...
            if reverse:
                records = [
                    msg for m in records for msg in m.to_letta_message(
    ...
    def get_agent_state(
            ...
            agent_name: Optional[str] = None) -> Optional[AgentState]:
        """Return the config of an agent"""
        user = self.user_manager.get_user_by_id(user_id=user_id)
        if agent_id:
            if self.ms.get_agent(agent_id=agent_id, user_id=user_id) is None:
            ...
            agent_state = self.ms.get_agent(agent_name=agent_name,
                                            user_id=user_id)
            if agent_state is None:
                raise ValueError(
                    f"Agent agent_name={agent_name} does not exist")
        ...
        agent_state.tags = self.agents_tags_manager.get_tags_for_agent(
            agent_id=agent_id, actor=user)
        return agent_state
    ...
    def update_agent_core_memory(self, user_id: str, agent_id: str,
                                 new_memory_contents: dict) -> Memory:
        """Update the agents core memory block, return the new state"""
        if self.user_manager.get_user_by_id(user_id=user_id) is None:
            raise ValueError(f"User user_id={user_id} does not exist")
        if self.ms.get_agent(agent_id=agent_id, user_id=user_id) is None:
        ...
        for key, value in new_memory_contents.items():
            if letta_agent.memory.get_block(key) is None:
                ...
                raise ValueError(
                    f"Key {key} not found in agent memory {str(letta_agent.memory.memory)}"
                )
            if value is None:
                continue
            if letta_agent.memory.get_block(key) != value:
                letta_agent.memory.update_block_value(
                    label=key, value=value)  # update agent memory
                modified = True
    ...
    def rename_agent(self, user_id: str, agent_id: str,
        ...
        if current_name == new_agent_name:
            raise ValueError(
                f"New name ({new_agent_name}) is the same as the current name")
    ...
    def delete_agent(self, user_id: str, agent_id: str):
        ...
        self.agents_tags_manager.delete_all_tags_from_agent(agent_id=agent_id,
                                                            actor=actor)
        ...
        if not agent_state:
            raise ValueError(
                f"Could not find agent_id={agent_id} under user_id={user_id}")
        if agent_state.user_id != user_id:
            raise ValueError(
                f"Could not authorize agent_id={agent_id} with user_id={user_id}"
            )
        ...
        try:
            self.active_agents = [
                d for d in self.active_agents
                if str(d["agent_id"]) != str(agent_id)
            ]
        except Exception as e:
            logger.exception(
                f"Failed to delete agent {agent_id} from cache via ID with:\n{str(e)}"
            )
        ...
        try:
            ...
            logger.exception(
                f"Failed to delete agent {agent_id} via ID with:\n{str(e)}")
    ...
    def create_api_key(
            self, request: APIKeyCreate) -> APIKey:  # TODO: add other fields
        """Create a new API key for a user"""
        if request.name is None:
            request.name = f"API Key {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        token = self.ms.create_api_key(user_id=request.user_id,
                                       name=request.name)
        return token
    ...
    def create_source(self, request: SourceCreate,
    ...
    def update_source(self, request: SourceUpdate, user_id: str) -> Source:
        ...
        if not request.id:
            existing_source = self.ms.get_source(source_name=request.name,
                                                 user_id=user_id)
        else:
    ...
    def delete_file_from_source(
            ...
            user_id: Optional[str]) -> Optional[FileMetadata]:
        return self.ms.delete_file_from_source(source_id=source_id,
                                               file_id=file_id,
                                               user_id=user_id)
    ...
    def load_data(
        ...
        passage_count, document_count = load_data(connector, source,
    ...
    def detach_source_from_agent(
        ...
        if not source_id:
            ...
            source = self.ms.get_source(source_name=source_name,
                                        user_id=user_id)
            source_id = source.id
        else:
            source = self.ms.get_source(source_id=source_id)
    ...
    def list_data_source_passages(self, user_id: str,
                                  source_id: str) -> List[Passage]:
        warnings.warn(
            "list_data_source_passages is not yet implemented, returning empty list.",
    ...
    def list_all_sources(self, user_id: str) -> List[Source]:
        ...
        for source in sources:
            ...
            passage_conn = StorageConnector.get_storage_connector(
                TableType.PASSAGES, self.config, user_id=user_id)
            ...
            attached_agents = [{
                ...
                str(a_id),
                "name":
                self.ms.get_agent(user_id=user_id, agent_id=a_id).name,
    ...
    def add_default_external_tools(self, actor: User) -> bool:
        ...
        for tool_create in tool_creates:
            try:
                self.tool_manager.create_or_update_tool(
    ...
    def get_agent_message(self, agent_id: str,
    ...
    def update_agent_message(self, agent_id: str,
    ...
    def rethink_agent_message(self, agent_id: str,
    ...
    def get_organization_or_default(self, org_id: str = "") -> Organization:
        ...
        try:
            return self.organization_manager.get_organization_by_id(
                org_id=org_id)
        except ValueError:
            raise HTTPException(
                status_code=404,
