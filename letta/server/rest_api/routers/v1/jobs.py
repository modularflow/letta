...
async def list_jobs(
    ...
    jobs = await server.list_jobs(user_id=actor.id)
...
async def list_active_jobs(
    ...
    return await server.list_active_jobs(user_id=actor.id)
...
async def get_job(
    ...
    return await server.get_job(job_id=job_id)
...
async def delete_job(
    ...
    """
    job = await server.get_job(job_id=job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")

    await server.delete_job(job_id=job_id)
    return job
