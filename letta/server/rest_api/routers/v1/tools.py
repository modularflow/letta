...
async def delete_tool(
    ...
    await server.tool_manager.delete_tool(tool_id=tool_id)
...
async def get_tool(
    ...
    tool = await server.tool_manager.get_tool_by_id(tool_id=tool_id, actor=actor)
    if tool is None:
        # return 404 error
...
async def get_tool_id(
    ...
    try:
        tool = await server.tool_manager.get_tool_by_name(tool_name=tool_name, actor=actor)
        return tool.id
...
async def list_tools(
    ...
    try:
        actor = await server.get_user_or_default(user_id=user_id)
        return await server.tool_manager.list_tools(actor=actor, cursor=cursor, limit=limit)
...
async def create_tool(
    ...
    return await server.tool_manager.create_or_update_tool(pydantic_tool=tool, actor=actor)
...
async def update_tool(
    ...
    """
    actor = await server.get_user_or_default(user_id=user_id)
    return await server.tool_manager.update_tool_by_id(tool_id=tool_id, tool_update=request, actor=actor)
...
async def add_base_tools(
    ...
    """
    actor = await server.get_user_or_default(user_id=user_id)
    return await server.tool_manager.add_base_tools(actor=actor)
