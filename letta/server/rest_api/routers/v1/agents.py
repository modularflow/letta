...
async def list_agents(
    ...
    agents = await server.list_agents(user_id=actor.id, tags=tags)
...
async def get_agent_context_window(
    ...
    return await server.get_agent_context_window(user_id=actor.id, agent_id=agent_id)
...
async def create_agent(
    ...
    return await server.create_agent(agent, actor=actor)
...
async def update_agent(
    ...
    actor = await server.get_user_or_default(user_id=user_id)
    return await server.update_agent(update_agent, actor=actor)


@router.get("/{agent_id}/tools", response_model=List[Tool], operation_id="get_tools_from_agent")
async def get_tools_from_agent(
    agent_id: str,
    server: "AsyncServer" = Depends(get_letta_server),
    user_id: Optional[str] = Header(None, alias="user_id"),  # Extract user_id from header, default to None if not present
):
    ...
    actor = await server.get_user_or_default(user_id=user_id)
    return await server.get_tools_from_agent(agent_id=agent_id, user_id=actor.id)
...
async def add_tool_to_agent(
    ...
    actor = await server.get_user_or_default(user_id=user_id)
    return await server.add_tool_to_agent(agent_id=agent_id, tool_id=tool_id, user_id=actor.id)
...
async def remove_tool_from_agent(
    ...
    """Add tools to an existing agent"""
    actor = await server.get_user_or_default(user_id=user_id)
    return await server.remove_tool_from_agent(agent_id=agent_id, tool_id=tool_id, user_id=actor.id)
...
async def get_agent_state(
    ...
    return await server.get_agent_state(user_id=actor.id, agent_id=agent_id)
...
async def delete_agent(
    ...
    return await server.delete_agent(user_id=actor.id, agent_id=agent_id)
...
async def get_agent_in_context_messages(
    ...
    return await server.get_in_context_messages(agent_id=agent_id)
...
async def get_agent_memory(
    ...
    return await server.get_agent_memory(agent_id=agent_id)
...
async def update_agent_memory(
    ...
    memory = await server.update_agent_core_memory(user_id=actor.id, agent_id=agent_id, new_memory_contents=request)
    return memory
...
async def get_agent_recall_memory_summary(
    ...
    return await server.get_recall_memory_summary(agent_id=agent_id)
...
async def get_agent_archival_memory_summary(
    ...
    return await server.get_archival_memory_summary(agent_id=agent_id)
...
async def get_agent_archival_memory(
    ...
    return await server.get_agent_archival_cursor(
        user_id=actor.id,
        agent_id=agent_id,
        after=after,
        before=before,
...
async def insert_agent_archival_memory(
    agent_id: str,
    ...
    return await server.insert_archival_memory(user_id=actor.id, agent_id=agent_id, memory_contents=request.text)
...
async def delete_agent_archival_memory(
    agent_id: str,
    memory_id: str,
    ...
    await server.delete_archival_memory(user_id=actor.id, agent_id=agent_id, memory_id=memory_id)
...
async def get_agent_messages(
    ...
    return await server.get_agent_recall_cursor(
        user_id=actor.id,
        agent_id=agent_id,
        before=before,
        limit=limit,
...
async def update_message(
    ...
    return await server.update_agent_message(agent_id=agent_id, request=request)
