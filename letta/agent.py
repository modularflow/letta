...
def compile_memory_metadata_block(
    ...
    timestamp_str = memory_edit_timestamp.astimezone().strftime(
        "%Y-%m-%d %I:%M:%S %p %Z%z").strip()
...
def compile_system_message(
    ...
    datetime,  # TODO move this inside of BaseMemory?
    archival_memory: Optional[ArchivalMemory] = None,
    recall_memory: Optional[RecallMemory] = None,
...
def initialize_message_sequence(
    ...
    first_user_message = get_login_event(
    )  # event letting Letta know the user just logged in
...
class Agent(BaseAgent):
    ...
    def __init__(
        ...
        if not any(
                isinstance(rule, TerminalToolRule)
                and rule.tool_name == send_message_terminal_rule.tool_name
                for rule in agent_state.tool_rules):
        ...
        assert isinstance(
            self.memory, Memory
        ), f"Memory object is not of type Memory: {type(self.memory)}"
        ...
        self.persistence_manager = LocalStateManager(
        ...
        if self.agent_state.message_ids is not None:
            ...
            printd(
                f"Agent.__init__ :: creating, state={agent_state.message_ids}")
            ...
            if initial_message_sequence is not None:
                ...
                init_messages = [
                    Message.dict_to_message(agent_id=self.agent_state.id,
                                            user_id=self.agent_state.user_id,
                                            model=self.model,
            ...
            assert all([
                isinstance(msg, Message) for msg in init_messages_objs
            ]), (init_messages_objs, init_messages)
        ...
        self.messages_total = messages_total if messages_total is not None else (
            len(self._messages) - 1)  # (-system)
    ...
    def link_tools(self, tools: List[Tool]):
        ...
        for tool_name in self.agent_state.tools:
            ...
            ], f"Tool name {tool_name} not included in agent tool list"
        ...
        for tool in tools:
            try:
                ...
                self.functions_python[tool.json_schema["name"]] = env[
                    tool.json_schema["name"]]
                self.functions.append(tool.json_schema)
            except Exception as e:
                warnings.warn(f"WARNING: tool {tool.name} failed to link")
                print(e)
        assert all([callable(f) for k, f in self.functions_python.items()
                    ]), self.functions_python

    def _load_messages_from_recall(self,
        ...
        for msg_id in message_ids:
            msg_obj = self.persistence_manager.recall_memory.storage.get(
                msg_id)
            if msg_obj:
                if isinstance(msg_obj, Message):
                    ...
                    printd(
                        f"Warning - message ID {msg_id} is not a Message object"
                    )
                    warnings.warn(
                        f"Warning - message ID {msg_id} is not a Message object"
                    )
                ...
                printd(
                    f"Warning - message ID {msg_id} not found in recall storage"
                )
                warnings.warn(
                    f"Warning - message ID {msg_id} not found in recall storage"
                )
    ...
    def refresh_message_buffer(self):
        ...
        assert messages_to_sync and all(
            [isinstance(msg_id, str) for msg_id in messages_to_sync])
    ...
    async def _prepend_to_messages(self, added_messages: List[Message]):
        ...
        new_messages = [
            self._messages[0]
        ] + added_messages + self._messages[1:]  # prepend (no system)
        self._messages = new_messages
    ...
    async def _get_ai_reply(
        ...
        try:
            ...
            if response.choices[0].finish_reason not in [
                ...
                raise Exception(
                    f"API call finish with bad finish reason: {response}")
    ...
    async def _handle_ai_response(
        ...
        if response_message_id is not None:
            assert response_message_id.startswith(
        ...
        if response_message.function_call or (
            ...
            if response_message.tool_calls is not None and len(
                    response_message.tool_calls) > 1:
            ...
            assert response_message.tool_calls is not None and len(
            ...
            function_call = (response_message.function_call
                             if response_message.function_call is not None else
            ...
            if isinstance(heartbeat_request,
                          str) and heartbeat_request.lower().strip() == "true":
                heartbeat_request = True

            if not isinstance(heartbeat_request,
                              bool) or heartbeat_request is None:
                printd(
    ...
    async def step(
        ...
        **kwargs,
        ...
        next_input_message = messages if isinstance(messages,
                                                    list) else [messages]
        counter = 0
            ...
            # Chain handlers
                ...
                next_input_message = Message.dict_to_message(
                    ...
                    openai_message_dict={
                        "role": "user",  # TODO: change to system?
                        "content":
                        get_heartbeat(FUNC_FAILED_HEARTBEAT_MESSAGE),
                    },
                )
                continue  # always chain
    ...
    async def inner_step(
        ...
        try:
            ...
            if ms is not None:
                should_update = False
                for block in self.agent_state.memory.to_dict(
                )["memory"].values():
            ...
            if len(input_message_sequence
                   ) > 1 and input_message_sequence[-1].role != "user":
                printd(
            ...
            if not skip_verify and (first_message or self.messages_total
                                    == self.messages_total_init):
                printd(
                ...
                while True:
                    ...
                    if verify_first_message_correctness(
                            response,
                            require_monologue=self.first_message_verify_mono):
                        break
            ...
            if self.agent_state.llm_config.context_window is None:
                ...
                self.agent_state.llm_config.context_window = (
                    ...
                    (self.model is not None and self.model in LLM_MAX_TOKENS)
    ...
    async def summarize_messages_inplace(self,
        ...
        printd(
            f"MESSAGE_SUMMARY_TRUNC_TOKEN_FRAC={MESSAGE_SUMMARY_TRUNC_TOKEN_FRAC}"
        )
        printd(
            f"MESSAGE_SUMMARY_TRUNC_KEEP_N_LAST={MESSAGE_SUMMARY_TRUNC_KEEP_N_LAST}"
        )
        ...
        if disallow_tool_as_first:
            while self.messages[cutoff]["role"] in [
                ...
                printd(
                    f"Selected cutoff {cutoff} was a 'tool', shifting one...")
                cutoff += 1

        message_sequence_to_summarize = self._messages[
            1:cutoff]  # do NOT get rid of the system message
        ...
        summary_message = package_summarize_message(summary,
        ...
        printd(
            f"Ran summarizer, messages length {prior_len} -> {len(self.messages)}"
        )
    ...
    async def rebuild_memory(self,
        ...
        curr_system_message = self.messages[
            0]  # this is the system + memory bank, not just the system prompt

        # NOTE: This is a hacky way to check if the memory has changed
        memory_repr = self.memory.compile()
        if not force and memory_repr == curr_system_message["content"][-(
                len(memory_repr)):]:
        ...
        if ms:
            for block in self.memory.to_dict()["memory"].values():
                ...
                if not isinstance(db_block.value, str):
                    printd(
                        f"skipping block update, unexpected value: {block_id=}"
                    )
                    continue
                # TODO: we may want to update which columns we're updating from shared memory e.g. the limit
                self.memory.update_block_value(label=block.get("label", ""),
        ...
        diff = united_diff(curr_system_message["content"],
...
async def update_state(self) -> AgentState:
    ...
    def attach_source(self, source_id: str, source_connector: StorageConnector,
                      ms: MetadataStore):
        ...
        page_size = 100
        generator = source_connector.get_all_paginated(
            filters=filters, page_size=page_size)  # yields List[Passage]
        all_passages = []
        for i in tqdm(range(0, size, page_size)):
            ...
            for passage in passages:
                assert isinstance(
                    passage, Passage
                ), f"Generate yielded bad non-Passage type: {type(passage)}"
            ...
            self.persistence_manager.archival_memory.storage.insert_many(
                passages)
            all_passages += passages

        assert size == len(
            all_passages
        ), f"Expected {size} passages, but only got {len(all_passages)}"
        ...
        ms.attach_source(agent_id=self.agent_state.id,
                         source_id=source_id,
                         user_id=self.agent_state.user_id)
    ...
    def update_message(self, request: UpdateMessage) -> Message:
        ...
        message = self.persistence_manager.recall_memory.storage.get(
            id=request.id)
        if message is None:
            raise ValueError(f"Message with id {request.id} not found")
        assert isinstance(
            message,
            Message), f"Message is not a Message object: {type(message)}"
        ...
        updated_message = self.persistence_manager.recall_memory.storage.get(
            id=message.id)
        if updated_message is None:
            raise ValueError(
                f"Error persisting message - message with id {request.id} not found"
            )
    ...
    def rethink_message(self, new_thought: str) -> Message:
        ...
        for x in range(len(self.messages) - 1, 0, -1):
            ...
            if msg_obj.role == MessageRole.assistant:
                updated_message = self.update_message(request=UpdateMessage(
                    id=msg_obj.id,
                    text=new_thought,
                ))
    ...
    def rewrite_message(self, new_text: str) -> Message:
        ...
        for x in range(len(self._messages) - 1, 0, -1):
            if self._messages[x].role == MessageRole.assistant:
                ...
                if message_obj.tool_calls is not None and len(
                        message_obj.tool_calls) > 0:
                    ...
                    message_obj.tool_calls[
                        0].function.arguments = new_args_string
    ...
    def pop_message(self, count: int = 1) -> List[Message]:
        ...
        if n_messages <= MIN_MESSAGES:
            ...
            for _ in range(min(count, len(self._messages))):
                ...
                try:
                    self.persistence_manager.recall_memory.storage.delete(
                        filters={"id": deleted_message.id})
                    popped_messages.append(deleted_message)
                except Exception as e:
                    warnings.warn(
                        f"Error deleting message {deleted_message.id} from recall memory: {e}"
                    )
    ...
    def retry_message(self) -> List[Message]:
        ...
        step_response = self.step_user_message(
            user_message_str=user_message.text)
        messages = step_response.messages

        assert messages is not None
        assert all(isinstance(msg, Message)
                   for msg in messages), "step() returned non-Message objects"
        return messages
    ...
    def get_context_window(self) -> ContextWindowOverview:
        ...
        if (len(self._messages) > 1
            ...
            num_tokens_messages = (num_tokens_from_messages(
                messages=messages_openai_format[2:], model=self.model)
                                   if len(messages_openai_format) > 2 else 0)
            ...
            num_tokens_messages = (num_tokens_from_messages(
                messages=messages_openai_format[1:], model=self.model)
                                   if len(messages_openai_format) > 1 else 0)
        ...
        num_tokens_external_memory_summary = count_tokens(
        ...
        if self.functions:
            ...
            num_tokens_available_functions_definitions = num_tokens_from_functions(
                functions=self.functions, model=self.model)
        else:
            available_functions_definitions = []
...
def save_agent(agent: Agent, ms: MetadataStore):
    ...
    assert isinstance(
        agent_state.memory,
        Memory), f"Memory is not a Memory object: {type(agent_state.memory)}"
...
def strip_name_field_from_user_message(
        user_message_text: str) -> Tuple[str, Optional[str]]:
