...
async def upload_file_using_client(client: Union[LocalClient, RESTClient], source: Source, filename: str) -> Job:
    # load a file into a source (non-blocking job)
    upload_job = await client.load_file_to_source(filename=filename, source_id=source.id, blocking=False)
    print("Upload job", upload_job, upload_job.status, upload_job.metadata_)

    # view active jobs
    active_jobs = await client.list_active_jobs()
    ...
    while True:
        status = (await client.get_job(upload_job.id)).status
